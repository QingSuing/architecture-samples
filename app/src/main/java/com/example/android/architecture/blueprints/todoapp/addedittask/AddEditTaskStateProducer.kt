...
private fun Flow<Action.UpdateTitle>.titleStateChanges(
): Flow<StateChange<AddEditTaskUiState>> = mapLatest {
    StateChange {
        copy(title = it.title)
    }
}

private fun Flow<Action.UpdateDescription>.descriptionStateChanges(
): Flow<StateChange<AddEditTaskUiState>> = mapLatest {
    StateChange {
